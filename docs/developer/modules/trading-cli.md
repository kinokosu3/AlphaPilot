...
TradingStrategySystem 的正式 CLI 适配层，覆盖定义、实例、preview、回放、独立部署、部署 observer 行情订阅、运行诊断、通用决策比较、kill switch、审计和本地 Broker UAT。
...
测试覆盖全部命令 help、JSON 解析、文件输出、操作员审计、生命周期和 UAT 金额/确认；CLI 数量由 contract test 固定为 120 个全局命令。
