...
class ExecutionEnvironment(str, Enum):
    """Physical execution environment, orthogonal to promotion level."""
...
class ExecutionBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if environment == ExecutionEnvironment.LOCAL_PAPER.value:
            ...
            profile = ""
        elif environment == ExecutionEnvironment.BROKER_SIMULATION.value and not profile:
            raise ValueError("BROKER_SIMULATION requires an account_profile alias")
        elif environment == ExecutionEnvironment.LIVE.value and data_kind != "realtime":
            raise ValueError("LIVE execution requires realtime quote data")
        ...
        if self.binding_hash and self.binding_hash != expected:
            raise ValueError("execution binding_hash does not match its immutable fields")
