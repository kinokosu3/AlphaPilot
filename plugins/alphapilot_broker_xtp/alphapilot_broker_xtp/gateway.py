...
Migrated from the original XTP gateway implementation to the XTP Pro / XTPX
1.2.1 SDK. Everything an XTP-family SDK shares — int
...
Only ``alphapilot_xtpx.api`` (compiled pybind bindings) is imported — no vn.py. On
...
try:  # compiled pybind bindings — present in the live env, absent on dev boxes
    from alphapilot_xtpx.api import MdApi as _SdkMdApi
    from alphapilot_xtpx.api import TdApi as _SdkTdApi
...
class XtpProGateway(AShareVendorGateway):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not roles or not roles <= {"trade", "quote"}:
            raise ValueError(f"invalid XTP gateway roles: {sorted(roles)}")
        self.roles = frozenset(roles)
        self.md_api = _XtpMdApi(self) if "quote" in roles else None
        self.td_api = _XtpTdApi(self) if "trade" in roles else None

    # ---- BrokerGateway ----------------------------------------------------- #
    def connect(self, setting: dict) -> None:
        if not SDK_AVAILABLE:
            raise ImportError(
                "broker 'xtp' needs the compiled 'alphapilot_xtpx.api' bindings; install the "
                "vendored alphapilot_xtpx package in the live environment (see docs/live-xtp.md)"
            )
        ...
        self.start(polling="trade" in self.roles)
    ...
    def send_order(self, req: OrderRequest) -> str:
        if self.td_api is None:
            raise RuntimeError("XTP gateway was created without the trade role")
    ...
    def subscribe(self, codes: list[str]) -> None:
        if self.md_api is None:
            raise RuntimeError("XTP gateway was created without the quote role")
        for raw in codes:
    ...
    def _convert_account(self, data: dict) -> Account | None:
        ...
        return account_from_xtp(data, account_id, self.name)
