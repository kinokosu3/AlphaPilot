...
Migrated from the original EMT gateway implementation onto the shared
:class:`~alphapilot_broker_xcommon.vendor_common.AShareVendorGateway`
...
from alphapilot_broker_xcommon.vendor_common import (
    AShareVendorGateway,
...
try:  # compiled pybind bindings — present in the live env, absent on dev boxes
    from alphapilot_emt.api import MdApi as _SdkMdApi
    from alphapilot_emt.api import TdApi as _SdkTdApi
...
def _md_native_logout_enabled() -> bool:
    return os.environ.get("ALPHAPILOT_LIVE_EMT_MD_LOGOUT", "").lower() in {"1", "true", "yes", "on"}
...
class EmtGateway(AShareVendorGateway):
    ...
    def __init__(
        ...
        name: str | None = None,
        *,
        roles: frozenset[str] = frozenset({"trade", "quote"}),
        **kwargs,
    ) -> None:
        super().__init__(name, **kwargs)
        if not roles or not roles <= {"trade", "quote"}:
            raise ValueError(f"invalid EMT gateway roles: {sorted(roles)}")
        self.roles = frozenset(roles)
        self.md_api = _EmtMdApi(self) if "quote" in roles else None
        self.td_api = _EmtTdApi(self) if "trade" in roles else None

    # ---- BrokerGateway ----------------------------------------------------- #
    def connect(self, setting: dict) -> None:
        if not SDK_AVAILABLE:
            raise ImportError(
                "broker 'emt' needs the compiled 'alphapilot_emt.api' bindings; install the "
                "vendored alphapilot_emt package in the live environment (see Dockerfile.live)"
            )
        if setting and ("trade" in setting or "quote" in setting):
            role_settings = setting
            setting = {}
            if "quote" in self.roles:
                setting.update(role_settings.get("quote") or {})
            if "trade" in self.roles:
                setting.update(role_settings.get("trade") or {})
        if not setting:
            ...
            if "quote" in self.roles:
                setting.update(build_quote_connect_setting(self.name))
            if "trade" in self.roles:
                setting.update(build_connect_setting(self.name))
    ...
    def close(self) -> None:
        if self.md_api is not None:
            self.md_api._closing = True
        if self.td_api is not None:
            self.td_api._closing = True
    ...
    def send_order(self, req: OrderRequest) -> str:
        if self.td_api is None:
            raise RuntimeError("EMT gateway was created without the trade role")
    ...
    def query_orders(self) -> bool:
        return self.td_api.query_orders() if self.td_api is not None else False

    def query_trades(self) -> bool:
        return self.td_api.query_trades() if self.td_api is not None else False

    def subscribe(self, codes: list[str]) -> None:
        if self.md_api is None:
            raise RuntimeError("EMT gateway was created without the quote role")
        for raw in codes:
    ...
    def _convert_account(self, data: dict) -> Account | None:
        if data.get("account_type") == 1 and self.td_api is not None:
            self.td_api.margin_trading = True
        account_id = self.td_api.userid if self.td_api is not None else ""
        return account_from_emt(data, account_id, self.name)
